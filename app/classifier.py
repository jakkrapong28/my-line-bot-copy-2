...
class QuestionClassifier:
    def __init__(self) -> None:
        ...
        self.pat_danger = re.compile(
            r"(น้ำมันเครื่อง).{0,40}(cvt|เกียร์\s*cvt)|(cvt).{0,40}(น้ำมันเครื่อง)", re.I
        )
        self.pat_spaces = re.compile(r"\s+")

    def safety_q(self, q: str) -> str | None:
        ql = q.lower()
...
DIRECT_ANSWERS: list[tuple[re.Pattern, str]] = [
    (
        re.compile(
            r"หาตัวแทนจำหน่าย|ขอทราบตัวแทน|อยากทราบตัวแทน|ค้นหาตัวแทน|ซื้อ.{0,6}ที่ไหนดี|จำหน่าย.{0,6}ที่ไหนบ้าง",
            re.I,
        ),
    ...
    (
        re.compile(
            r"จารบี|ไฮดรอลิก|สินค้าอุตสาหกรรม|compressor.{0,6}oil|gear.{0,6}oil.{0,6}อุตสาหกรรม", re.I
        ),
    ...
    (
        re.compile(
            r"ค้นหาน้ำมัน.{0,6}รถ|เช็คน้ำมัน.{0,6}รถ|น้ำมัน.{0,6}เหมาะ.{0,10}รถ|รถ.{0,6}ใช้น้ำมัน.{0,6}อะไร",
            re.I,
        ),
